...
function Profil() {
  ...
  const { dataUser, dataActivity, dataAverageSessions, dataPerformance, dataPerformanceKind, isLoading, error } = GetDataFetch()
    
        if (error) {
          return <div className='message-oups'> <img className='imageOups' src={imageOups} alt="image404" /> </div>;
        } else if (isLoading) {
          return <Loader />
        } else {
            return (
                <div className="profil">
                    ...
                    <React.StrictMode>
                        <ProfilUser 
                      name={dataUser.data.userInfos.firstName}
                        />
                        <div className="dashboard">
                              ...
                              <div className="nutrient">
                                    {Object.keys(dataUser.data.keyData).map((key) => (
                                      <NutrientCard categorie={key} value={dataUser.data.keyData[key]} key={key} />))}
                              </div>
